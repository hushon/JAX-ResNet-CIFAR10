...
def main():
    ...
    normalize = ArrayNormalize(FLAGS.CIFAR10_MEAN, FLAGS.CIFAR10_STD)
    ...
    def eval_step(train_state: TrainState, batch: dict):
        params, state, _, _ = jax.tree_map(lambda x: x[0], train_state)
        input, target = batch['image'], batch['label']
    ...
    for epoch in trange(FLAGS.MAX_EPOCH, bar_format='{l_bar}{bar:10}{r_bar}{bar:-10b}', smoothing=1.):
        for input, target in train_loader:
            batch = {
                'image': einops.rearrange(input, '(n b) h w c -> n b h w c', n=n_devices),
                'label': einops.rearrange(target, '(n b) -> n b', n=n_devices),
            }
        ...
        tprint(f'[{epoch}/{FLAGS.MAX_EPOCH}] LR: {last_lr:.3f} | Train Loss {train_loss.mean():.3f} | Test Loss {loss:.3f} Acc: {acc:.3f}')
